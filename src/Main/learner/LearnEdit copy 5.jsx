...
const LearEdit = () => {
      ...
      try {
        ...
        fileKeys.forEach(key => {
          if (data[key]) {
            ...
            const url = driveId ? `${BURL}/api/image-proxy/${driveId}` : `${data[key]}?t=${Date.now()}`;
            previews[key] = {
              url,
              isPDF: key !== "photo" // only photo is guaranteed to be image
            };
          }
        });
  ...
  const renderFileInput = (name, label, onlyImage = false) => {
    ...
    const isPDF = preview?.isPDF || file?.type === "application/pdf";
