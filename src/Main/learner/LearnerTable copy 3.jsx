...
const LearnerTable = () => {
  ...
  useEffect(() => {
    if (!fromDate || !toDate || !toDateConfirmed) return;
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (!axios.isCancel(err)) {
          ...
          if (
            err.response &&
            (err.response.status === 401 || err.response.data.message === "Invalid token")
          ) {
            setTimeout(() => {
  ...
  }, [search, selectedGender, fromDate, toDate, toDateConfirmed, currentPage]);
  ...
  const handleFromDateChange = (e) => {
    ...
    setToDate("");
    setToDateConfirmed(false);
    updateURLParams({ search, gender: selectedGender, fromdate: val, todate: "", page: 1 });
    setCurrentPage(1);
  };
    ...
    <div className="p-4">
      <div className="flex flex-col gap-4 mb-4 md:flex-row md:items-center md:justify-between">
        ...
        <button
          ...
          className="w-full px-4 py-2 text-white bg-blue-500 rounded-md hover:bg-blue-600 md:w-auto"
        >
          Register
        </button>
      </div>
