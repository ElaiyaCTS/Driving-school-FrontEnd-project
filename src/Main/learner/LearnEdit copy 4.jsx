...
const LearEdit = () => {
    ...
    const fetchLearner = async () => {
      try {
        ...
        fileKeys.forEach(key => {
          if (data[key]) {
            const driveId = extractDriveFileId(data[key]);
            previews[key] = {
              url: driveId ? `${BURL}/api/image-proxy/${driveId}` : `${data[key]}?t=${Date.now()}`
            };
          }
        });
  ...
  const renderFileInput = (name, label, onlyImage = false) => {
    ...
    const openModal = () => setModalState(p => ({ ...p, [name]: true }));
    const closeModal = () => setModalState(p => ({ ...p, [name]: false }));
        ...
        {isOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="relative w-11/12 max-w-2xl p-6 bg-white rounded-lg">
              <button onClick={closeModal} className="absolute text-red-500 top-2 right-2">✕</button>
              {isPDF ? (
