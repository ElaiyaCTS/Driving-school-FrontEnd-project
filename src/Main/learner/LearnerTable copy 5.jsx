 ...
 useEffect(() => {
  ...
  const fetchData = async () => {
    ...
    try {
     ...
     params.limit = 10; // always include limit
  ...
  const handleFromDateChange = (e) => {
    ...
    if (val === "") {
      setToDate(""); // ✅ Clear toDate when fromDate is cleared
    }
  ...
  const handleToDateChange = (e) => {
    ...
    if (val === "") {
      setToDate(""); // ✅ Allow clearing toDate
      updateURLParams({
        search,
        gender: selectedGender,
        fromdate: fromDate,
        ...
        <div className="flex flex-col w-full gap-4 sm:flex-row sm:items-center md:w-auto md:justify-end">
          <div className="relative w-full sm:w-40">
            ...
            <label
              ...
              className={`absolute text-xs left-3 top-[-8px] bg-white px-1 text-gray-500 peer-focus:text-blue-600 ${selectedGender ? "text-blue-600" : ""}`}
            >
              Gender
            </label>
          </div>
